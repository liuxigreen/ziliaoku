...
5. **1-2 张竖版小黑插画 `xhs_illus*_*_3x4.png`**：`ian-xiaohei-illustrations` 风（白底手绘小黑），**须 3:4 竖版**（小红书笔记是竖图轮播流；公众号同 IP 插画是 16:9 横图，两处格式不同）。画核心反差/痛点场景，放封面+文字卡片之后作额外 slide——统一跨平台 IP、把抽象概念画出来促存图/转评。← 与公众号插画同 IP，小红书版多这一步格式适配。
...
2. **写双版本 md**（各自守禁则）。
3. **配图**：PIL 生成双方封面/卡片/头图；ImageGen 生成插图（ian-xiaohei-illustrations）——公众号正文插图 16:9 横图 + **小红书竖版小黑插画 3:4**（同 IP 两种格式）。
...
9. **交付预览（发图给用户，必做）**：所有资产生成 + 脱钩校验通过后，**用 present_files 把 小红书封面 + 竖版小黑插画 + 公众号头图/封面 + 正文插图（小黑）发出来给用户预览**。每篇都必须"发"出来看，不能只在 repo 里生成完就结束——这是用户明确要求的收尾动作（曾发生过只发 skill/清单、漏发实际图的情况）。
...
- 小红书小黑插画**须 3:4 竖版**（非 16:9 横图），否则在竖图轮播流里被压成小横条难看。
