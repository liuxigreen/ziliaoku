...
- **高风险诉求 → 合规替代动作（来自 launch-pack 合规转化替代方案，2026-07-10 新增）**：上面的词表管"已写出来的红线词"，下面这张管"用户/原文想做的违规动作"——遇到这些诉求，不直接做，改成合规替代并给话术。
  | 高风险诉求 | 合规替代动作 |
  |---|---|
  | 隐藏/绕开联系方式、谐音绕检测 | 拒绝；改成平台内行动引导（评论提问→平台内答），或用官方店铺/企业号/平台批准线索工具 |
  | 买阅读/刷量/互赞互粉 | 拒绝；转向标题/选题/完读率/复盘机制优化，提醒人为互动会污染受众信号 |
  | 复制对标文章还不被发现 | 拒绝洗稿；只抽结构，用原创资料+自有证明重建 |
  | 图片/素材直接拿搜索结果 | 要求合法授权/自有图/合规截图/生成授权素材 |
  | 低阅读就注销重来 | 先诊断违规/定位漂移/质量，有明确战略理由才重开 |
  | 设计绕过平台审核的方法 | 拒绝；改成合规的内容+行动引导系统 |
  推荐话术（命中时引用）：
  - "我不能帮你设计绕过平台审核的方法，但可以帮你改成合规的平台内行动引导。"
  - "对标只学结构，不搬原文。我们用你的素材和证明重新写。"
  - "涉及平台规则或政策敏感内容时，先核验当前平台社区规则。"
