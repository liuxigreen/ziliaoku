# -*- coding: utf-8 -*-
"""opencli 选题库 · 公众号深读版 → 摸鱼绿排版 HTML（复用 gzh_style，解析 md 自动渲染）。
输出: gzh_assets/opencli_topic_library_wechat_排版_摸鱼绿(moyu-green).html
...
SRC = 'D:/WorkBuddyProjects/ziliaoku/output/posts/2026-07-16/opencli_topic_library/opencli_topic_library_wechat.md'
OUT_DIR = 'D:/WorkBuddyProjects/ziliaoku/output/posts/2026-07-16/opencli_topic_library/gzh_assets'

COVER = {
    'tag': 'AI 工作流 · 译介实测',
    't1': '别再手动刷热点了',
    't2': '0 元搭个选题库',
    'sub': '用 opencli 免费直采大佬动态，做一手信源',
    'src': '个人创作者可复制',
    'tag_a': 'opencli', 'tag_b': '免费直采',
}
INTRO_LEAD = '做内容最贵的不是工具，是「今天写什么」的决策质量。'
...
def main():
    ...
    while i < len(lines):
        ...
        if line.startswith('> '):
            ...
            i += 1; continue
        if line.startswith('🤖') or line.startswith('【想复现'):
            flush()
    ...
    out = os.path.join(OUT_DIR, 'opencli_topic_library_wechat_排版_摸鱼绿(moyu-green).html')
