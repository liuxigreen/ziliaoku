#!/usr/bin/env python3
"""生成 opencli 选题库 双版配图（纯 PIL，零成本、中文不乱码）。
...
def gzh_header():
    ...
    # 标题（两行）
    for i, ln in enumerate(wrap("0 元搭选题库：把大佬动态变成一手信源", F(44, True), W-300, d)):
    ...
    img.save(os.path.join(OUT, "gzh_header_opencli.png"))
    print("gzh header done")


if __name__ == "__main__":
    cover()
    card(1, "每天想「今天写啥」？", "算法只喂你二手货\n等你刷到 已经写过 30 篇", "xhs_card1_opencli_3x4.png")
    card(2, "opencli 免费直采", "复用浏览器登录态\n拉大佬 X/Reddit 0 元不封号", "xhs_card2_opencli_3x4.png")
    card(3, "我的四步落地", "列名单 → 拉动态 → 关键词过滤\n→ 每天早上扫一眼挑能写的", "xhs_card3_opencli_3x4.png")
    gzh_header()
    print("ALL DONE")
