...
> 你以为在指挥 AI，其实是别人在代码里藏了句话指挥 AI。

之前我写过一篇讲「用 AI 读代码被反杀」的短文，当时只讲了现象。这次重读，发现那个坑比我想的更深——它不是个例，而是一类正在被安全圈反复验证的真实攻击。这篇把**机制、三个可查证的真实案例、还有打工人当天能落地的防御**一次讲透。

## 一句话先记住

**你以为在教 AI 读代码，其实是代码在教 AI 坑你。**

AI 编程助手分不清「你写的指令」和「代码里藏的指令」。一旦它有了"跑命令、读文件、装包"的手，别人藏在注释、文档、PR 里的那句话，就能反过来指挥它替攻击者干活。

## PART 01 真事：安全团队真验过的劫持
...
- **Clinejection（2026.02，研究员 Adnan Khan）**：攻击者在一个 GitHub issue 的**标题**里藏指令，劫持了项目官方的自动处理机器人。最终约 4000 台机器装上了带后门的 Cline CLI。整个过程从"一个 issue 标题"一路走到"供应链投毒"，中间没人点任何链接。
...
你会发现，三个案例的**攻击入口都是"一段别人写的文本"**，而 AI 老老实实把它当成了指令。区别只在于这段文本藏在代码、issue 标题，还是 MCP 工具描述里。

## PART 02 机制：为什么代码能"反过来指挥"AI
...
一句话：**传统 prompt injection 骗的是"读文字的人"，而 Agent 的 prompt injection 骗的是"能动手的 Agent"——后者约等于远程代码执行。**

## PART 03 攻击面：打工人天天踩的 3 个坑

别觉得"我是小透明，没人盯我"。你最容易中招的，恰恰是"用 AI 处理别人的东西"：
...
## PART 04 防御：5 招把 AI 关进沙箱

不用吓得不敢用 AI，5 招就能把风险压到很低，而且都是打工人当天能做的：
...
一句话收尾：**AI 是工具，但工具读的东西可能是带毒的。**

## 我的不同意见：别因噎废食，但要长记性

我写这个不是劝你别用 AI 读代码——恰恰相反，我自己天天用。关键是**建立"不信任输入"的肌肉记忆**：你喂给 AI 的每一样东西，都先默认"它可能藏着指令"。
